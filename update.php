...
<html lang="ja">
    ...
    <body>
        ...
        <main>
            ...
            <form method="post" name="form" action="update_confirm.php">
            <?php while ($row = $stt->fetch()) { ?>
                <input type="hidden" value="<?php print($_POST['id'])?>" name="id">
                <div>
                    <label>名前(姓)</label>
                    <input type="text" size="35" maxlength="10" name="last"  value="<?php print($row['family_name'])?>" pattern="[\u4E00-\u9FFF\u3040-\u309F-]*">
                </div>
                <div id="a"></div>

                <div>
                    <label>名前(名)</label>
                    <input type="text" size="35" maxlength="10" name="first"  value="<?php print($row['last_name'])?>" pattern="[\u4E00-\u9FFF\u3040-\u309F-]*">
                </div>
                <div id="b"></div>

                <div>
                    <label>カナ(姓)</label>
                    <input type="text" size="35" maxlength="10" name="kanala"  value="<?php print($row['family_name_kana'])?>" pattern="[\u30A1-\u30F6]*">
                </div>
                <div id="c"></div>

                <div>
                    <label>カナ(名)</label>
                    <input type="text" size="35" maxlength="10" name="kanafi"  value="<?php print($row['last_name_kana'])?>" pattern="[\u30A1-\u30F6]*">
                </div>
                <div id="d"></div>

                <div>
                    <label>メールアドレス</label>
                    <input type="text" size="35" maxlength="100" name="mail" value='<?php print($row['mail'])?>'  pattern="^[0-9A-Za-z-@.]+$">
                </div>
                <div id="e"></div>

                <div>
                    <label>パスワード</label>
                    <input type="text" size="35" maxlength="10" name="pass" value='<?php echo str_repeat("●",mb_strlen($row['password'],"UTF8"));?>'  pattern="^[a-zA-Z0-9]+$">
                </div>
                <div id="f"></div>

                <div>
                    <label>性別</label>
                    <input type="radio" name="man"  value="男" <?php if($row['gender']==0){echo 'checked';} ?> >男
                    <input type="radio" name="man" value="女" <?php if($row['gender']==1){echo 'checked';} ?> >女
                </div>
                <div id="g"></div>

                <div>
                    <label>郵便番号</label>
                    <input type="text" size="35" maxlength="7" name="postcode"  value="<?php print($row['postal_code'])?>" pattern="^[0-9]+$">
                </div>
                <div id="h"></div>
                ...
                <div>
                    <label>住所(都道府県)</label>
                    <select name="address" >
                        <option value="<?php print($row['prefecture'])?>"><?php print($row['prefecture'])?></option>
                ...
                <div>
                    <label>住所(市区町村)</label>
                    <input type="text" size="35" maxlength="10" name="municipalities" value='<?php print($row['address_1'])?>' pattern="[-\u4E00-\u9FFF\u3040-\u309F―\uFF66-\uFF9F\u30A1-\u30F60-9 ０-９_\s]*">
                </div>
                <div id="j"></div>
                
                <div>
                    <label>住所(番地)</label>
                    <input type="text" size="35" maxlength="100" name="banti" value='<?php print($row['address_2'])?>' pattern="[-\u4E00-\u9FFF\u3040-\u309F―\uFF66-\uFF9F\u30A1-\u30F60-9 ０-９_\s]*">
                </div>
                <div id="k"></div>
